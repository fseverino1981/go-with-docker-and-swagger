...
func TestUserControllerInterface_FindUserByEmail(t *testing.T) {
	...
	t.Run("email_is_invalid_returns_error",
		func(t *testing.T) {
	...
	t.Run("email_is_valid_service_ returns_error",
		func(t *testing.T) {
			recorder := httptest.NewRecorder()
			context := GetTestGinContext(recorder)
	...
	t.Run("email_is_valid_service_ returns_success",
		func(t *testing.T) {
			recorder := httptest.NewRecorder()
			context := GetTestGinContext(recorder)
			...
			service.EXPECT().FindUserByEmailServices("test@test.com").Return(
				model.NewUserDomain("test@test.com", "test", "User Test", 20), nil)
